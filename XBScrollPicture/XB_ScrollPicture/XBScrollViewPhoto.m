...
- (instancetype)initWithFrame:(CGRect)frame withImgsArray:(NSArray*)imgsArray clickPhontIndex:(phototBlock)photoIndex{
    if (self = [super initWithFrame:frame]) {
        ...
        for (int i = 0; i < imgsArray.count; i++) {
            NSString *imgUrlName =  imgsArray[i];
            [[XBWebImgManager shareWebImgManager] downloadImageWithUrlString:imgUrlName];
        }
...
//      NSLog(@"%ld>>>>>>>%ld>>>>>>>>%ld",left,centent,right);
    if ([self isNetworkResources]) {
        self.leftImage.image = [self setImageUrlNameWithIndex:left];
        self.centerImage.image = [self setImageUrlNameWithIndex:centent];
        self.rightImage.image = [self setImageUrlNameWithIndex:right];
    }else{
...
//网络图片
- (UIImage *)setImageUrlNameWithIndex:(NSInteger)index {
    
    //从内存缓存中取,如果没有使用占位图片
    UIImage *image = [[[XBWebImgManager shareWebImgManager] webImageCache] valueForKey:self.imgsArray[index]];
    image = image ? image : self.placeImage;
    return image ;
}
...
- (void)clickTime{
    [self.scrollew setContentOffset:CGPointMake(self.scrollew.contentOffset.x + XBScrollView_Width, 0) animated:YES];
}
